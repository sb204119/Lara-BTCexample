...
class VendorController extends Controller
{
    ...
    public function p_bg(Request $request){

        if ($request->hasFile('file')) {
            $user = auth()->user()->vendor;
            $destinationPath = public_path('storage/avatars/vendor/');
            $fileName = $user->id . '.jpg';

            $request->file('file')->move($destinationPath, $fileName);
            $user->profilebg = 'storage/avatars/vendor/' . $fileName;
            $user->save();
    ...
    public function addOffer(NewOfferRequest $request, Product $product = null)
    {
        ...
        try {
            ...
            session() -> flash('success', 'Предложение добавленно!');
        } catch (RequestException $e){
